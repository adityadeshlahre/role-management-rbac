...
cleanup() {
  ...
  if docker ps -a --filter "name=role-rbac-full" --format '{{.Names}}' | grep -q '^role-rbac-full$'; then
    echo "Stopping and removing application container..."
    docker stop role-rbac-full
    docker rm role-rbac-full
  fi
